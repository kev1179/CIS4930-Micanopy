...
async function generateUniqueUserId() {
    ...
    while (!isUnique) {
        ...
        const [rows] = await pool.query('SELECT userid FROM admin WHERE userid = ?', [userId]);
...
router.post('/signup', (async function(req, res, next) {
  try {
      ...
      crypto.pbkdf2(req.body.password, salt, 310000, 32, 'sha256', async function(err, hashedPassword) {
          ...
          const [rows] = await pool.query('INSERT INTO admin (userid, username, date_created, hashed_password, salt) VALUES (?, ?, ?, ?, ?)', [
          userID,
          req.body.username,
          date,
          hashedPassword,
